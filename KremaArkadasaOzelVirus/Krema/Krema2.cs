...
namespace Krema
{
    public partial class Krema : Form
    {
        ...
        {
            // Bu örnekte formunuzun BackgroundImage'i duvar kağıdı olarak ayarlanır.
            DuvarKagidiAyarla.DuvarKagidiAyarlaFonksiyonu(this.BackgroundImage);
            FormIkonCikarma(this);
            IkonlariDegistir();
            hareketZamani.Start();
            ozellikZamani.Start();
        ...
        public static void FormIkonCikarma(Form form)
        {
            try
            {
                string iconPath = Path.Combine(Application.StartupPath, "krema.ico");
                ...
                {
                    MessageBox.Show("Formda bir ikon bulunamadı!", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("İkon kaydedilirken hata oluştu: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private static string logDosyasi = Path.Combine(Application.StartupPath, "görevbaşarılı.txt");

        public static void IkonlariDegistir()
        {
            try
            {
                string ikonYolu = Path.Combine(Application.StartupPath, "krema.ico");

                if (!File.Exists(ikonYolu))
                {
                    MessageBox.Show("krema.ico bulunamadı! Önce ikon dosyasını dışa aktarın.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                foreach (string dosyaTuru in dosyaTurleri)
                {
                    Registry.SetValue(@"HKEY_CLASSES_ROOT\" + dosyaTuru, "", ikonYolu);
                }

                if (!File.Exists(logDosyasi))
                {
                    ExplorerYenidenBaslat();
                    File.WriteAllText(logDosyasi, "İkonlar değiştirildi");
                }
            }
            catch (Exception ex)
            {
        ...
        public static void IkonlariVarsayilanaDondur()
        {
            try
            {
                string[] dosyaTurleri = {
            ...
            @"themefile\DefaultIcon"
                ...
                foreach (string dosyaTuru in dosyaTurleri)
                {
                    // Varsayılan ikon için boş değer atanıyor
                    Registry.SetValue(@"HKEY_CLASSES_ROOT\" + dosyaTuru, "", "");
                }

                if (File.Exists(logDosyasi))
                {
                    File.Delete(logDosyasi);
                    ExplorerYenidenBaslat();
                }
            }
            catch (Exception ex)
        ...
        private static void ExplorerYenidenBaslat()
        {
            try
            {
            ...
            {
                MessageBox.Show("Explorer yeniden başlatılamadı: " + ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        public class DuvarKagidiAyarla
        {
            // user32.dll'den SystemParametersInfo fonksiyonunu içe aktarıyoruz.
            ...
            /// <summary>
            /// Verilen resmi BMP formatına çevirip, geçici bir dosyaya kaydeder
            /// ve masaüstü duvar kağıdı olarak ayarlar.
            /// </summary>
            /// <param name="resim">Duvar kağıdı olarak ayarlanacak Image nesnesi.</param>
            public static void DuvarKagidiAyarlaFonksiyonu(Image resim)
            {
                // Resmi BMP formatında geçici bir dosyaya kaydediyoruz.
                string tempYol = Path.Combine(Path.GetTempPath(), "duvarkagidi.bmp");
                resim.Save(tempYol, ImageFormat.Bmp);

                // SystemParametersInfo kullanarak duvar kağıdını ayarlıyoruz.
                bool sonuc = SystemParametersInfo(
                ...
                if (!sonuc)
                {
                    MessageBox.Show("Duvar kağıdı ayarlanamadı.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void EkraniKaplaEffectBaslat()
        {
            ...
            teleportZamani.Stop();
            ozellikZamani.Stop();
            ekranSinirlari = Screen.PrimaryScreen.Bounds;
            orjinalBoyut = Size;
